...
const openOrSenior = (data) => {
  const rank = data.map((x) => (x[0] >= 55 && x[1] > 7 ? "Senior" : "Open"));
  return rank;
};
...
// forma alternativa con destructuring

function openOrSenior1(data) {
  return data.map(
    (
      [age, handicap] // Accede a los 2 valores de cada array que se itera y les pone un nombre de variable temporal
